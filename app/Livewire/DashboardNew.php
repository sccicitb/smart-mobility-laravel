...
class DashboardNew extends Component
{
    ...
    public function fetchData()
    {
        ...
        try {
            $this->data_emisi = Cache::remember($key, now()->addMinutes(10), function () use ($filter) {
            ...
            ")
                ...
                $result = $query->first();
                $emisi = $result->total_emisi_co2_kg ?? 0;
                $cost  = $result->total_kerugian_ribu_rp ?? 0;

                if ($emisi < 1000 && $cost < 50000)          $los = 'A';
                elseif ($emisi < 5000 && $cost < 200000)     $los = 'B';
                elseif ($emisi < 10000 && $cost < 500000)    $los = 'C';
                elseif ($emisi < 20000 && $cost < 1000000)   $los = 'D';
                else                                          $los = 'E';

                if ($filter === 'today') {
                    $start = $end = today()->toDateString();
                } elseif ($filter === 'week') {
                ...
                $peak = DB::select("
                    WITH volume_per_jam AS (
                        SELECT DATE(waktu) AS tanggal, HOUR(waktu) AS jam,
                    ...
                    rentang_3jam AS (
                        SELECT v1.jam AS jam_mulai, v1.jam + 2 AS jam_akhir,
                            v1.total_volume + COALESCE(v2.total_volume,0) + COALESCE(v3.total_volume,0) AS total_rentang
                        FROM volume_per_jam v1
                        LEFT JOIN volume_per_jam v2 ON v1.tanggal = v2.tanggal AND v2.jam = v1.jam + 1
                        LEFT JOIN volume_per_jam v3 ON v1.tanggal = v3.tanggal AND v3.jam = v1.jam + 2
                    )
                    SELECT CONCAT(LPAD(CAST(jam_mulai AS CHAR),2,'0'),':00') AS start_jam,
                           CONCAT(LPAD(CAST(jam_akhir AS CHAR),2,'0'),':59') AS end_jam
                    FROM rentang_3jam ORDER BY total_rentang DESC LIMIT 1
                ", [$start, $end]);

                $peakRange = (!empty($peak) && isset($peak[0]))
                    ? $peak[0]->start_jam . " - " . $peak[0]->end_jam
                    : null;
            ...
            // Selalu sinkron dari cache — closure mungkin tidak jalan jika hit cache
            $this->cost     = $this->data_emisi['cost'] ?? 0;
            $this->los      = $this->data_emisi['los'] ?? '';
            $this->peakTime = $this->data_emisi['peak'] ?? '';
    ...
    public function setFilter($filter)
    {
        ...
        // Invalidate cache untuk filter ini agar data terbaru diambil
        Cache::forget("emisi_{$this->filter}");
        Cache::forget("vehicle_{$this->filter}");
    ...
    public function loadVehicleData()
    {
        ...
        $data = Cache::remember("vehicle_{$this->filter}", now()->addMinutes(10), function () use ($start, $end) {
            return VehicleQuery::getVehicleData($start, $end);
        });
        ...
        $this->dispatch('bar-chart-updated',
            ...
            data: $this->vehicleData['incomingVehicles'] ?? [],
            positionText: true
        );
        $this->dispatch('bar-chart-updated',
            id: 'bar-chart-outgoing',
            data: $this->vehicleData['outgoingVehicles'] ?? [],
            positionText: false
        );
    }
    ...
    public function mount()
    {
        ...
        // Data dimuat saat component sudah di-render (lazy)
        $this->loadData();
        $this->loadVehicleData();
        $this->fetchData();
    }
    ...
    // hydrate dihapus: menyebabkan re-fetch di setiap Livewire network request
